...
inline boost::future<void> wamp_session::start()
{
    ...
    boost::asio::dispatch(m_io_service, [this, weak_self]() {
        ...
        const std::map<std::string, std::string>& authentication_extra)
{
    ...
    boost::asio::dispatch(m_io_service, [this, weak_self, message]() {
        auto shared_self = weak_self.lock();
        if (!shared_self) {
            return;
        }
...
inline boost::future<void> wamp_session::publish(const std::string& topic,const wamp_publish_options& options)
{
    ...
    boost::asio::dispatch(m_io_service, [this, weak_self, message, result]() {
        auto shared_self = weak_self.lock();
        if (!shared_self) {
            return;
        }
        ...
        const wamp_subscribe_options& options)
{
    ...
    boost::asio::dispatch(m_io_service, [this, weak_self, message, request_id, subscribe_request]() {
        auto shared_self = weak_self.lock();
        if (!shared_self) {
            return;
        }
...
inline boost::future<void> wamp_session::unsubscribe(const wamp_subscription& subscription)
{
    ...
    boost::asio::dispatch(m_io_service, [this, weak_self, message, request_id, unsubscribe_request]() {
        auto shared_self = weak_self.lock();
        if (!shared_self) {
            return;
        }
        ...
        const wamp_call_options& options)
{
    ...
    boost::asio::dispatch(m_io_service, [this, weak_self, message, request_id, call]() {
        auto shared_self = weak_self.lock();
        if (!shared_self) {
            return;
        }
        ...
        const provide_options& options)
{
    ...
    boost::asio::dispatch(m_io_service, [this, weak_self, message, request_id, register_request]() {
        auto shared_self = weak_self.lock();
        if (!shared_self) {
            return;
        }
...
inline boost::future<void> wamp_session::unprovide(const wamp_registration& registration)
{
	...
	boost::asio::dispatch(m_io_service, [this, weak_self, message, request_id, unregister_request]() {
		auto shared_self = weak_self.lock();
		if (!shared_self) {
			return;
		}
...
inline void wamp_session::process_challenge(wamp_message&& message)
{
    ...
    (*context_response) = on_challenge(challenge_object).then([this, context_response]( boost::future<wamp_authenticate> fu_auth) {
        try {
            ...
            auto weak_self = std::weak_ptr<wamp_session>(this->shared_from_this());
            boost::asio::dispatch(m_io_service, [this, weak_self, message, context_response]() {
                auto shared_self = weak_self.lock();
                if (!shared_self) {
                    return;
                }
...
inline void wamp_session::process_invocation(wamp_message&& message)
{
    ...
    if (procedure_itr != m_procedures.end()) {
        ...
        auto send_result_fn = [weak_this] (const std::shared_ptr<wamp_message>& message) {
            ...
            boost::asio::dispatch(shared_this->m_io_service, [weak_this, message] {
                auto shared_this = weak_this.lock();
                if (!shared_this) {
                    return; // FIXME: or throw exception?
                }
